...
function loadStatsPage(){
	...
	for (row = 2; row < table.rows.length; row++){
		if (parseInt(table.rows[row].cells[2].innerHTML) < parseInt(table.rows[row].cells[3].innerHTML)) {
			score = table.rows[row].cells[3].innerHTML;
			table.rows[row].cells[4].innerHTML = "Opponent";
			opp++;
			console.log(score)
		}
		else {
			score = table.rows[row].cells[2].innerHTML;
			table.rows[row].cells[4].innerHTML = "Home";
			home++;
			console.log(score)
		}
		//table.rows[row].cells[4].innerHTML = score;
	}
	document.getElementById("wins").innerHTML = home;
	document.getElementById("losses").innerHTML = opp;
...
function loadPlayersPage(){
	...
	for(i = 0; i < players.length; i++){
		...
		a.setAttribute("class", "btn-primary dropdown-item");
		//a.onclick = switchPlayers(i);
		//a.setAttribute("onclick", "switchPlayers(${i})");
		a.onclick=function(i){return function(){switchPlayers(i);}}(i);

		select_player.appendChild(a);
	}
}
